...
local C = {
    ...
    SupportedGames = {
        -- Add games like this:
        {Id = 12355337193, Name = "Murderers VS Sheriffs DUELS", Exec = function()
            loadstring(game:HttpGet("https://raw.githubusercontent.com/tbao143/thaibao/main/TbaoHubMurdervssheriff"))()
        end},
        {Id = 2788229376, Name = "Dahood - some errors...", Exec = function()
...
local function StartLoader()
        ...
        if VerifyKey(UI.KeyInput.Text) then
            if SaveKey(UI.KeyInput.Text) then
                FakeLoad(UI,function()
                    ...
                    local gameData
                    for _,g in pairs(C.SupportedGames) do if g.Id==gId then gameData=g break end end
                    if gameData then
                        ...
                        pcall(gameData.Exec)
                        wait(1.5)
                        Tween(UI.Container,{Size=UDim2.new(0,0,0,0),Position=UDim2.new(0.5,0,0.5,0),BackgroundTransparency=1}).Completed:Wait()
                        UI.Gui:Destroy()
                    else
                        UI.Status.Text = "GAME NOT SUPPORTED\nRECOMMEND? in discord??"
                        UI.Status.TextColor3 = C.Colors.Error
                        UI.UniversalHubBtn.Visible = true
                    end
                end)
            else
    ...
    UI.UniversalHubBtn.MouseButton1Click:Connect(function()
        FakeLoad(UI,function()
            UI.Status.Text = "EXITING..."
            UI.Status.TextColor3 = C.Colors.Primary
            pcall(function()
